...
- 公開用コンテナは原則 `ports:` を使わず、`devops-edge` + `expose:` で内部公開する。（例外: DBのメンテ用など、必要時のみホストpublishを許可する）
- LLM は「提案生成」に使い、反映は差分レビューしてから行う（危険領域は人間が最終判断）。
...
- DevOps-Studio のドキュメント（ARCHITECTURE, PROJECT_CHARTER）は LLM への「制約条件」としても活用する。
...
1. Astraeus で開発開始。
   - Cursor + Continue + Ollama を使って設計・実装を進める。
